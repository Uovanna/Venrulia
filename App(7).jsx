...
const withRng = (fn, body) => { const prev = _rng; _rng = fn; try { return body(); } finally { _rng = prev; } };
...
// XP per level. Retention-tuned shape (exponent 2.6): very fast early, moderate mid, long
// endgame grind toward 60 (~50h total) — level 60 is the gateway to true endgame.
const xpForLevel = (lvl) => Math.floor(15 * Math.pow(lvl, 2.6));
const MAX_LEVEL = 60;
// after level 60, surplus XP becomes Honor; each Honor level grants 1 attribute point
const honorXpForLevel = (h) => Math.floor(xpForLevel(MAX_LEVEL) * (1 + h * 0.12));
    ...
    <div style={{ background: "#100e1c", border: `1px solid ${r.color}55`, borderLeft: `3px solid ${r.color}`, borderRadius: 8, padding: "9px 11px", display: "flex", alignItems: "center", gap: 10 }}>
      <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0, cursor: onClick ? "pointer" : "default" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: r.color, fontWeight: 700, fontSize: 12.5, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.enchant ? "✨ " : ""}{item.name}</div>
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: "linear-gradient(180deg,#0c0a18,#08060f)", border: `2px solid ${r.color}`, borderRadius: 10, padding: "14px 16px", maxWidth: 300, width: "100%", boxShadow: `0 0 26px ${r.color}55` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ color: r.color, fontWeight: 700, fontSize: 15, fontFamily: "Georgia, serif", lineHeight: 1.15 }}>{item.enchant ? "✨ " : ""}{item.name}</div>
        ...
        <div style={{ borderTop: "1px solid #241f3c", margin: "7px 0", paddingTop: 6 }}>
          {mainKeys.filter((k) => merged[k] > 0).map((k) => <div key={k} style={{ color: "#fff", fontSize: 12 }}>+{merged[k]} {STAT_LABEL[k]}</div>)}
          {secKeys.filter((k) => merged[k] > 0).map((k) => <div key={k} style={{ color: "#4ade80", fontSize: 12 }}>+{merged[k]} {STAT_LABEL[k]}</div>)}
...
function GameScreen({ character: initChar, onSave, onBack }) {
  ...
  const syncArtifacts = (c) => {
    ...
    let touched = false;
    const fix = (it) => { if (it?.artifact && it.ilvl !== want) { touched = true; return makeArtifact(c.cls, it.slotId, c.level, it); } return it; };
  ...
  const startHard = (inst, kind) => {
    ...
    if (kind === "dungeon") {
      if (dungeonRunsLeft(c, inst.id) <= 0) { const left = dungeonResetLeft(c, inst.id); showNotif(`⏳ Out of runs — resets in ${fmtClock(left)}`); return; }
  ...
  // A single push of a Mail message onto a working char draft.
  const pushMail = (draft, msg) => ({ ...draft, mail: [{ id: uid(), createdAt: Date.now(), ...msg }, ...(draft.mail || [])].slice(0, 100) });

  // ---- market sweep: runs in the background AND on open ----
  // Processes expiries, resolves phantom BUYERS for the player's own listings,
  // and tops the phantom SUPPLY back up so shelves are never bare (never flooded).
  // Entirely backend; nothing here is surfaced to the player as "system"/"bot".
  const ahSweep = (force = false) => {
    const c = charRef.current; if (!c) return;
    const nowMs = Date.now();
    const last = c.ahMeta?.lastSweep || 0;
    if (!force && nowMs - last < AH_ECON.sweepMinMs) return;
    const elapsedHr = Math.min(72, (nowMs - (last || nowMs)) / 3600000); // cap offline catch-up window
    let draft = { ...c, ahListings: [...(c.ahListings || [])], mail: [...(c.mail || [])] };
    const keep = [];
    for (const L of draft.ahListings) {
      if (L.mine) {
        if (nowMs >= L.expiresAt) {
          // unsold → return goods via Mail (deposit already spent)
          draft = pushMail(draft, { kind: "expired", subject: L.kind === "gear" ? L.item.name : `${stackMeta(L.kind === "drop" ? "drop" : "mat", L.matId).name} ×${L.qty}`, from: "Auction House", gold: 0, gross: 0, tax: 0, ...(L.kind === "gear" ? { item: L.item } : { matKind: L.kind, matId: L.matId, qty: L.qty }) });
          continue;
        }
        // phantom demand: did a buyer take it during the elapsed window?
        const ratio = L.price / Math.max(1, L.base);
        const pSold = 1 - Math.pow(1 - sellChancePerHour(ratio), elapsedHr);
        if (Math.random() < pSold) {
          const net = ahNetAfterTax(L.price);
          draft = pushMail(draft, { kind: "sale", subject: L.kind === "gear" ? L.item.name : `${stackMeta(L.kind === "drop" ? "drop" : "mat", L.matId).name} ×${L.qty}`, from: ahSeller(), gold: net, gross: L.price, tax: ahTaxOf(L.price), qtySold: L.kind === "gear" ? 1 : L.qty });
          continue;
        }
        keep.push(L);
      } else {
        // phantom supply expires on its own clock, then gets topped up below
        if (nowMs < L.expiresAt) keep.push(L);
      }
    }
    // top up phantom stock — gentle refill (never bare), capped per sweep (never flooded)
    const gearN = keep.filter((L) => !L.mine && L.kind === "gear").length;
    const stackN = keep.filter((L) => !L.mine && L.kind !== "gear").length;
    const topUp = (have, target) => have < target / 2 ? target - have : Math.min(AH_ECON.refillPerSweep, target - have);
    for (let i = 0; i < topUp(gearN, AH_ECON.stockGear); i++) keep.push(makePhantomGear(nowMs));
    for (let i = 0; i < topUp(stackN, AH_ECON.stockStacks); i++) keep.push(makePhantomStack(nowMs));
    draft.ahListings = keep;
    draft.ahMeta = { lastSweep: nowMs };
    commitChar(draft);
  };
  // background sweep every 15s, plus an immediate sweep when opening AH or Mail
  useEffect(() => { const iv = setInterval(() => ahSweep(false), 15000); return () => clearInterval(iv); }, []);
  useEffect(() => { if (tab === "auction" || tab === "mail") ahSweep(true); }, [tab]);

  // ---- posting ----
  const postGear = (item, price) => {
    const c = charRef.current;
    ...
    const listing = { id: uid(), kind: "gear", item, price: p, base, seller: "You", mine: true, postedAt: Date.now(), expiresAt: Date.now() + AH_ECON.listDurationMs };
    commitChar({ ...c, gold: c.gold - fee, inventory: c.inventory.filter((i) => i.id !== item.id), ahListings: [...(c.ahListings || []), listing] });
    setAhSell(null); setAhPrice(""); setAhView("mine");
    showNotif(`Listed ${item.name} · −${fee}g deposit`);
  };
  const postStack = (kind, id, price) => {
    const c = charRef.current;
    ...
    const fee = ahPostFee(base);
    if (c.gold < fee) { showNotif(`Need ${fee}g deposit to list.`); return; }
    const listing = { id: uid(), kind, matId: id, qty: AH_ECON.stackSize, price: p, base, seller: "You", mine: true, postedAt: Date.now(), expiresAt: Date.now() + AH_ECON.listDurationMs };
    commitChar({ ...c, gold: c.gold - fee, [pool]: { ...c[pool], [id]: have - AH_ECON.stackSize }, ahListings: [...(c.ahListings || []), listing] });
    setAhSell(null); setAhPrice(""); setAhView("mine");
    showNotif(`Listed ${stackMeta(kind === "drop" ? "drop" : "mat", id).name} ×${AH_ECON.stackSize} · −${fee}g deposit`);
  };
  // buy a (phantom) listing → gold leaves the economy; goods arrive via Mail
  const buyAh = (L) => {
    const c = charRef.current;
    if (L.mine) return;
    if (c.gold < L.price) { showNotif("Not enough gold!"); return; }
    let draft = { ...c, gold: c.gold - L.price, ahListings: (c.ahListings || []).filter((x) => x.id !== L.id) };
    draft = pushMail(draft, { kind: "purchase", subject: L.kind === "gear" ? L.item.name : `${stackMeta(L.kind === "drop" ? "drop" : "mat", L.matId).name} ×${L.qty}`, from: L.seller, gold: 0, gross: L.price, tax: 0, ...(L.kind === "gear" ? { item: L.item } : { matKind: L.kind, matId: L.matId, qty: L.qty }) });
    commitChar(draft);
    showNotif(`Bought · ${L.price}g — check Mail 📬`);
  };
  // pull your own listing early → goods returned immediately (deposit already spent)
  const cancelAh = (L) => {
    const c = charRef.current; if (!L.mine) return;
    let inv = c.inventory, mats = c.materials, drops = c.drops;
    if (L.kind === "gear") inv = [...inv, L.item].slice(-120);
    else if (L.kind === "drop") drops = { ...drops, [L.matId]: (drops[L.matId] || 0) + L.qty };
    else mats = { ...mats, [L.matId]: (mats[L.matId] || 0) + L.qty };
    commitChar({ ...c, inventory: inv, materials: mats, drops, ahListings: (c.ahListings || []).filter((x) => x.id !== L.id) });
    showNotif("Listing cancelled — goods returned.");
  };
  // ---- mail ----
  const collectMail = (m) => {
    const c = charRef.current;
    let inv = c.inventory, mats = c.materials, drops = c.drops, gold = c.gold + (m.gold || 0);
    if (m.item) inv = [...inv, m.item].slice(-120);
    if (m.matId) { if (m.matKind === "drop") drops = { ...drops, [m.matId]: (drops[m.matId] || 0) + m.qty }; else mats = { ...mats, [m.matId]: (mats[m.matId] || 0) + m.qty }; }
    commitChar({ ...c, gold, inventory: inv, materials: mats, drops, mail: (c.mail || []).filter((x) => x.id !== m.id) });
  };
  const collectAllMail = () => {
    const c = charRef.current;
    let inv = [...c.inventory], mats = { ...c.materials }, drops = { ...c.drops }, gold = c.gold;
    for (const m of (c.mail || [])) {
      gold += (m.gold || 0);
      if (m.item) inv.push(m.item);
      if (m.matId) { if (m.matKind === "drop") drops[m.matId] = (drops[m.matId] || 0) + m.qty; else mats[m.matId] = (mats[m.matId] || 0) + m.qty; }
    }
    commitChar({ ...c, gold, inventory: inv.slice(-120), materials: mats, drops, mail: [] });
  ...
  const mailCount = (char.mail || []).length;
  ...
  return (
    <div style={{ height: "100dvh", minHeight: "100vh", background: "#08080f", color: "#e8e0d0", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {compareItem && (() => {
        ...
        const equipped = char.equipment[bag.slotId] || null;
          ...
          <div onClick={() => setCompareItem(null)} style={{ position: "fixed", inset: 0, background: "#000b", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "linear-gradient(180deg,#15122e,#0d0a1f)", border: "2px solid #46407a", borderRadius: 16, padding: 18, maxWidth: 420, width: "100%" }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "stretch" }}>
                <Panel item={equipped} title="Equipped" />
              ...
              <div style={{ marginTop: 10, background: "#0c0a1c", border: "1px solid #2a2550", borderRadius: 10, padding: "9px 12px" }}>
                <div style={{ color: "#8a83b8", fontSize: 9.5, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>If you equip the bag item</div>
                {(() => {
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                {char.inventory.some((i) => i.id === bag.id) && <button onClick={() => { equipItem(bag); setCompareItem(null); }} style={{ flex: 1, background: `linear-gradient(135deg,#1a2410,#22331a)`, border: `1.5px solid ${cls?.color || "#7CFC9E"}`, borderRadius: 10, color: cls?.color || "#7CFC9E", fontSize: 13, fontWeight: 700, padding: 10, cursor: "pointer" }}>Equip this</button>}
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: "auto", padding: tab === "town" ? "4px" : 14 }}>
                  ...
                  {HARD_DUNGEONS.map((hd) => {
                    ...
                    return (
                      <div key={hd.id} style={{ background: "#0e0c1a", border: `1.5px solid ${done ? "#5fd35f" : unlocked ? "#ff4500" : "#241f3c"}`, borderRadius: 10, padding: 11, opacity: unlocked || done ? 1 : 0.6 }}>
                        ...
                        <button disabled={!canRun} onClick={() => startHard(hd, "dungeon")} style={{ ...hardBtn(canRun), marginTop: 8 }}>{!unlocked ? (hd.reqIlvl && avg < hd.reqIlvl ? `🔒 Requires ilvl ${hd.reqIlvl}` : (hd.prevZone && !char.hardZoneDone?.[hd.prevZone]) ? `🔒 Complete ${hardZoneById(hd.prevZone)?.name}` : `🔒 ${HARD_BOSS_REQ} ${hd.prevBoss} kills`) : battle ? "Finish current fight first" : runsLeft <= 0 ? "⏳ No runs left" : "🔥 Enter Hard Dungeon"}</button>
                      </div>
                    );
                  })}
                </div>
        ...
        {tab === "auction" && (() => {
          ...
          const listings = char.ahListings || [];
          const f = ahFilters;
          const matchGear = (it) => {
          ...
          const buyable = listings.filter((L) => !L.mine && (ahCat === "gear" ? L.kind === "gear" : L.kind !== "gear"));
        ...
        {tab === "mail" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              ...
              {(char.mail || []).length > 0 && <MiniBtn onClick={collectAllMail} color="#7CFC9E" bg="#122015">Collect All</MiniBtn>}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {(char.mail || []).length === 0 && <div style={{ color: "#555", fontSize: 12, textAlign: "center", padding: 24 }}>Your mailbox is empty.</div>}
              {(char.mail || []).map((m) => {
...
const mpProvider = {
  ...
  findOpponent: (targetPower, level) => mpBot(targetPower, level),
        ...
        {done || !item ? (
          ...
          <div style={{ background: "#0e0c1a", border: `2px solid ${rc(item.rarity).color}`, borderRadius: 12, padding: 12, marginBottom: 10, textAlign: "left" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ color: rc(item.rarity).color, fontSize: 14, fontWeight: 700, lineHeight: 1.15 }}>{item.enchant ? "✨ " : ""}{item.name}</div>
...
function MultiplayerHub({ char, commitChar, showNotif, onExit, onStartRated }) {
  ...
  const buildLadder = (drift) => {
    if (drift) mpRosterDrift();
    const bots = mpLadderRoster(24).map((b) => ({ ...b, rated: { wins: b.wins, losses: b.losses }, rating: arenaRating(b.wins, b.losses) }));
    const meEntry = { ...me, rated: lifetime, rating: myRating };
    const field = bots.concat([meEntry]).sort((a, b) => b.rating - a.rating);
  ...
  const findMatch = () => {
    ...
    const opp = { ...mpProvider.findOpponent(myPower, char.level), rating: oppRating };
  ...
  return (
    <div style={{ paddingBottom: 20 }}>
      ...
      <div style={{ background: "#1a1330", border: "1px solid #46407a", borderRadius: 8, padding: "6px 10px", marginBottom: 10, color: "#b9a7e0", fontSize: 10, lineHeight: 1.4, textAlign: "center" }}>Arena — Ladder & Rated PvP. Opponents are simulated for now; live players slot in later.</div>
...
export default function App() {
  ...
  return <>{body}{cloudButton}{cloudOverlay}{conflictModal}</>;
}
